...
# function to find the round numbers that add up to n
def find_round_numbers(n):
    round_numbers = []
    while n > 0:
        # calculate the number of digits in the current number
        digits = count_digits(n)
        # calculate the current round number
        round_number = (n // (10 ** (digits - 1))) * (10 ** (digits - 1))
        # add the round number to the list
        round_numbers.append(round_number)
        # subtract the round number from n
        n -= round_number
...
if __name__ == '__main__':
    ...
    for i in range(t):
        # read the value of n
        n = int(input())
        # find the round numbers that add up to n
        round_numbers = find_round_numbers(n)
        # print the number of round numbers and the round numbers themselves
        print(len(round_numbers))
        print(' '.join(str(x) for x in round_numbers))
